...
**\`statement\`** — a one-line yes/no question a reader answers by voting Yes or No, about what the article is about. It is shown as the headline of a vote, on a phone screen, so it is short: **twelve words at most**, ending in a question mark, one clause, no "and" joining two ideas. Keep only the core — who did what — and leave detail to the description. Every article with readable text receives one, of one of two kinds, recorded in \`kind\`:
...
- **\`claim\`** — where \`decisions\` is empty (an accident, a profile, a sports record, a human-interest piece). Turn the article's central reported claim into a yes/no question, without adding a judgement or a decision nobody reported — "Did a club drop Juancho Hernangómez at 15 but keep his brother?" Do not open with "According to…" or name the outlet: the reader already sees which outlet published it. Set \`decision_id\` to null.
- Name the actor where the article names one.
- The question carries no endorsement and presumes no answer: a reasonable reader could answer Yes or No. Never ask about a predicted consequence — "Will the expansion cut emissions?" asks the reader to guess an outcome, not to judge what was reported.
- No evaluative adjectives, no leading words ("really", "finally", "even"), and no negative questions ("Shouldn't…?"). Where the article shows a dispute over terms, do not adopt either side's vocabulary.
