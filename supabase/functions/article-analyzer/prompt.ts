...
**\`statement\`** — a one-line yes/no question that a Barcelona resident answers by voting Yes or No. It is the headline of the vote, read on a phone in a second or two, and its job is to make the reader want to answer. So it asks for the reader's **view**, never for a fact they would have to look up: anyone should be able to answer it from their own opinion, without having read the article. Write it in English, whatever the article's language. It is short: **twelve words at most**, ending in a question mark, one clause, no "and" joining two ideas. Write it in plain, concrete, everyday words — name the real thing at stake (the fine, the rent, the hospital, the club, the street), not abstractions ("measures", "initiatives", "the situation"). Leave detail to the description. Every article with readable text receives one, of one of two kinds, recorded in \`kind\`:

- **\`decision\`** — where \`decisions\` is not empty. Ask about the primary decision. Anchor it to an entry in \`decisions\` and record that entry's \`id\`; where the article reports several, use the one it leads with. For something proposed or contested, ask whether it should happen — "Should London expand the ULEZ to outer boroughs?" For something already done, ask whether it was the right call, without judging it — "Should Transport for London have expanded the ULEZ to outer boroughs?"
- **\`claim\`** — where \`decisions\` is empty (an accident, a profile, a sports record, a human-interest piece). Do not ask whether the reported event happened: "Did a club drop Juancho Hernangómez at 15 but keep his brother?" is a fact check no reader can vote on. Instead ask the opinion question the story puts in front of a reader — the issue it raises, stated so both answers are reasonable. For example, a profile of a chef who quit a Michelin-starred kitchen might become "Are Michelin stars worth the pressure they put on kitchens?", and a report on a record heatwave in Seville might become "Should outdoor work stop during extreme heat?" These illustrate the move only; write each question from its own article, never reuse them. The issue must come from the article itself — its subject, its dispute, or the choice it describes — never from a topic you bring to it. Do not name the outlet or open with "According to…": the reader already sees which outlet published it. Set \`decision_id\` to null.
- Name the actor where the article names one.
- The question carries no endorsement and presumes no answer: a reasonable resident could answer Yes or No, and the wording must not tell them which. Never ask about a predicted consequence — "Will the expansion cut emissions?" asks the reader to guess an outcome, not to give a view.
- No evaluative adjectives, no leading words ("really", "finally", "even", "just"), no emotional hooks, and no negative questions ("Shouldn't…?"). Keep the article's own neutral terms for people and groups; do not swap them for narrower or more charged ones ("clients" does not become "men"). Where the article shows a dispute over terms, do not adopt either side's vocabulary.
- Carry an evidence tier, as with any other substantive output.

A \`claim\` statement is not a licence to invent: the issue it asks about comes from what the usable text reports, and where the text is thin it stays close to it. \`statement\` is null only when there is no readable article text at all.
